...
//    @IBOutlet weak var notificationButton: UIBarButtonItem!
    ...
    @objc func openMicrosubHelp(_ sender:UITapGestureRecognizer){
        if let openUrl = URL(string: "https://indiepass.app/ios/help/#microsub"),
    ...
    func updateSearchResults(for searchController: UISearchController) {
        ...
        if let searchText = searchController.searchBar.text, !searchText.isEmpty {
            UserDefaults(suiteName: AppGroup)?.set(ChannelListFilter.Search(searchText: searchText).value, forKey: "ChannelFilter")
            currentFiltering = .Search(searchText: searchText)
        } else {
            UserDefaults(suiteName: AppGroup)?.set(standardFiltering.value, forKey: "ChannelFilter")
...
//            if let storedTheme = (UserDefaults(suiteName: "group.software.studioh.indigenous")?.value(forKey: SelectedThemeKey) as AnyObject).integerValue
            UserDefaults(suiteName: AppGroup)?.set(ChannelListFilter.UnreadOnly.value, forKey: "ChannelFilter")
        ...
        alert.addAction(UIAlertAction(title: "All Channels", style: .default, handler: { [weak self] action in
            UserDefaults(suiteName: AppGroup)?.set(ChannelListFilter.None.value, forKey: "ChannelFilter")
...
//        notificationButton.image = UIImage.fontAwesomeIcon(name: .globe, textColor: UIColor.black, size: CGSize(width: 30, height: 30))
        ...
        if  micropubAccounts.count >= activeAccount + 1,
            ...
            if micropubDetails.microsub_endpoint != nil {
                ...
                if let savedFilter = UserDefaults(suiteName: AppGroup)?.string(forKey: "ChannelFilter") {
                ...
                if let microsubPage = onboardingStoryboard.instantiateViewController(withIdentifier: "onboardingTemplate") as? OnboardingTemplateViewController {
                    ...
                    microsubPage.contentText = "You can only read posts within IndiePass if your website supports Microsub. However, no microsub endpoint was detected on your website. Tap to see more information on settings up Microsub."
